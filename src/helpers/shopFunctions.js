import {
  removeCartID, saveCartID, priceCart, priceCartRemove, savedPriceStorage,
} from './cartFunctions';
...
export const createCartProductElement = ({ id, title, price, pictures }) => {
  ...
  li.addEventListener('click', () => {
    ...
    const priceNegative = price;
    const finalPrice = priceCartRemove() - priceNegative;
    totalPriceElement.innerHTML = finalPrice.toFixed(2);
    savedPriceStorage(finalPrice.toFixed(2));
  });
  return li;
};
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async () => {
    ...
    const item = createCartProductElement(data);
    cartProducts.appendChild(item);
    const totalPrice = priceCart(price);
    totalPriceElement.innerHTML = totalPrice;
    savedPriceStorage(totalPrice);
  });
